export const ADVANCEMENT_RULE_SOURCE = Object.freeze({
...
export const EXPERIENCE_TABLES = Object.freeze({
...
export const EXPERIENCE_PER_LEVEL_AFTER_TABLE = Object.freeze({
    fighter: 250000,
    paladin: 350000,
    ranger: 325000,
    cleric: 225000,
...
export const HIT_DIE_PROGRESSION = Object.freeze({
...
export function getExperienceThreshold(classId, level) {
    const normalizedLevel = normalizeLevel(level);
    const table = EXPERIENCE_TABLES[classId];
    if (normalizedLevel <= table.length)
        return table[normalizedLevel - 1] ?? null;
...
export function getLevelForExperience(classId, experience) {
    const xp = normalizeExperience(experience);
    let level = 1;
    while (true) {
        const nextThreshold = getExperienceThreshold(classId, level + 1);
        if (nextThreshold === null || xp < nextThreshold)
            return level;
        level += 1;
    }
}
export function getExperienceToNextLevel(classId, level, currentExperience) {
...
export function getAdvancementSnapshot(classId, level) {
    const normalizedLevel = normalizeLevel(level);
    return {
        classId,
