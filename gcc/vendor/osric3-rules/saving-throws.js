...
export const SAVING_THROW_LABELS = Object.freeze({
...
export const SAVING_THROW_RULE_SOURCE = Object.freeze({
...
export function getBestMulticlassSavingThrows(classes) {
    if (classes.length === 0)
        throw new RangeError('At least one class is required.');
    const rows = classes.map(({ classId, level }) => getSavingThrows(classId, level));
    return SAVING_THROW_IDS.map((_, index) => Math.min(...rows.map((row) => row[index] ?? Infinity)));
}
