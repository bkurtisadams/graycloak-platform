...
- **[15.4] Bombing.** An airborne creature carrying bombs has persistent Maneuverability Class one worse. Each man-sized weight-equivalent dropped makes one 2d6 attack per eligible represented flying creature, with the victim treated as AC 10 from Low altitude, AC 8 from Medium, or AC 4 from High regardless of its actual AC. Ordinary bombs do not inherit the carrier's poison, paralysis, level drain, magic/silver attack quality, or other natural-attack riders. Special or magical bomb effects are recorded before play and left to their definition/referee rather than inferred. v0.64.0.
- **[15.4] Drop procedure — BOARD PROCEDURE.** The printed rule gives no phase timing or horizontal footprint test. A drop therefore uses the carrier's Missile entry after movement, consumes its missile action for the round, and requires the flying unit/counter's centre to lie over the selected ground target. A mass unit keeps one uniform load per represented creature, and every represented creature drops the same chosen number of weight-equivalents; the chosen amount is expended from that uniform load. Exhausting the load removes the carrying penalty. v0.64.0.
- **[15.4] Scale boundary.** Individual-against-individual bombing uses normal §9.4B d20/hit-point resolution with §15.4's fixed target AC. Mass-unit bombing against a 1:1 target and bombing a mounted 1:1 pair remain deferred because the supplied rules do not say how to bridge the scale or select rider versus mount. v0.64.0.
...
- **Not yet built:** the D&D size-based turn table (the alternative to MC), the DMG 50%/75% wing-damage rules (BS [15.3]'s any-damage landing rule supersedes them for BATTLESYSTEM play), dragon breath vs moving aerial targets (+2 to saves), and any board display of altitude beyond the existing counter chip.
...
- **[14.12]/[14.13] Defences and movement magic** are RAW (v0.56.11 / v0.56.13).
- **[14.14/PHB] Illusions and Phantasmal Force — v0.68.1.** Printed illusion spells retain the persistent described marker and BATTLESYSTEM disbelief sequence. Phantasmal Force now distinguishes Illusionist 1 from Magic-User 3, deriving range and total square-game-inch area from caster level and converting the equal-area square through Table 17. It is visual only: a force must be recorded as an observer (with clear LOS) before it is affected or can disbelieve. The marker tracks concentration; caster movement, caster loss/damage, or voluntary cessation ends the spell. The caster may move the area inside spell range. The referee can record phantasmal damage or a hazard against an observing believer, and can resolve a strike/interaction by recording an appropriate reaction or ending the spell. A communicated allied disbelief applies the printed +4 bonus to associates' saves. Exact invented imagery and non-damage consequences remain referee judgments. `BattlesystemMagicIllusions`, `state.illusions`, `illusionObserve`, `applyPhantasmalDamage`, `movePhantasmalForce`.
- **[14.14/PHB] Advanced phantasms and terrain — v0.68.2.** Improved Phantasmal Force permits half-speed movement under minimal concentration, prevents other spellcasting, includes minor non-speech sounds, and lingers two rounds after concentration. Spectral Force adds sound, smell, and thermal components and lingers three rounds. Permanent Illusion creates the same full-sensory effect without concentration and persists until dispelled. Hallucinatory Terrain uses separate Illusionist 3 and Magic-User 4 range, casting-time, and square-area formulas; it grants no save and ends automatically when an intelligent battlefield figure contacts its area, or manually when the referee records Dispel Magic. Real terrain continues to govern movement and combat.
- **[14.14/PHB] Communicated disbelief and Dispel Magic — v0.68.3.** An allied successful disbelief unlocks the printed later retry, but associates receive the +4 saving-throw bonus only after the referee explicitly records effective communication from a force that disbelieved. That side-level communication state persists in saves. Illusion-marker dialogs now locate a selected caster's recorded Dispel Magic, apply its PHB class range plus normal timing, LOS, action, preparation, and slot rules, then let the referee record the source-dependent dispel check as successful or unsuccessful; either confirmed result expends the spell outside Setup sandbox.
- **Not yet implemented:** [14.15]/[14.17]/[14.18] referee-managed spells, [14.21] psionics.
- **[14.16] Curative magic — v0.65.0.** Cure light/serious/critical cure 1 HD per spell level; heal, regeneration, restoration and resurrection 2 HD per level; the marker clears once the HD cured *by such magic in total* reaches ½ the figure's HD, so the board keeps a running total on the marker (`u.woundHealedHd`) and resets it when the marker goes. hp-based cures restore hp ÷ 4 HD. Raise Dead returns a dead individual at 1 hp under an indefinite Immobilized status. Limited Wish / Alter Reality remove one marker; Wish removes the marker if one is present, otherwise brings one lost figure back. On a 1:1 individual the normal AD&D hp expression applies. **BOARD PROCEDURE:** the rulebook gives no touch geometry for a mass figure, so touch cures use a 1″ board adjacency (`touchRangeIn`). `BattlesystemMagicHealing`, `magicResolveHealingUse`.
- **[14.19] Clerics and undead — v0.65.0.** Turning resolves on the OSRIC 3.0 kernel already used by the dungeon sim (`gcc/vendor/osric3-rules/turn-undead.js`): one d20 against Table 1.6.5a, automatic T/D, the kernel's creature dice for the number affected, 3d4 rounds turned, paladins turn as clerics two levels lower from 3rd. The undead type is recorded on the unit (Special abilities ▸ Undead); Skeleton, Zombie and Ghoul roster rows carry it. BATTLESYSTEM adds: at least one figure is routed (or destroyed) even when the count falls short of a figure — figures affected = max(1, ⌊creatures ÷ creatures-per-figure⌋); one attempt per undead unit per Game Round until an attempt fails, after which that cleric may not try that unit again. Turned figures detach as their own routed body (the [2.9] split) under a `fear`-kind status for the duration, which already blocks Rally [14.9]; if every figure is affected the whole unit routs. **RULING (open to veto):** an evil cleric's success rolls the kernel's d100 — 61+ commands the figures instead, modelled as a mass charm with the cleric as Unit Commander and his CR as the control range [14.6A], which lapses when they leave his CR; below 61 they are turned normally. Turning is a Missile & Magic action (☩ Turn beside ✦ Magic) and spends the figure's magic action [14.1]. **BOARD PROCEDURE (v0.66.1, Kurt):** the rules give turning no range; the accepted 6″ is used, measured edge to edge. The turning level reads the character's class/level fields and the spell roster's class line, whichever records the cleric or paladin level.
- **[14.20] Magic items — v0.65.0.** A commander's active Ring of Human Influence adds +1 CB and a Rod of Rulership +2, read from the inventory. Drums of Panic (used through the Magic window) force an immediate Morale Check on every enemy mass unit within the DMG 12″ radius, the 1″ inner zone excepted. Sphere of Annihilation's 2″ push remains referee-managed. Rod of Victory / Rod of Ruling are D&D-game items and are not modelled.
- **[14.22] Bards — RULING (open to veto), v0.65.0.** A 1:1 bard who is not commanding is taken to be singing: friendly mass units within his CR check morale at +1, itemised on the morale line; one bard per unit; a commanding bard does not sing. There is no separate Sing verb — if a bard must be silent, make him a commander or remove him.
...
## §16 Campaign rules — v0.66.0

- **[16.2] Kill ledger.** Every casualty path credits the attacker (unit or individual) with creatures removed × the victim's XP kill value per creature (the v0.62.7 roster field); a unit that routs off the table credits the unit that caused the rout with its surviving creatures. Units with no XP value contribute 0 and are named in the result. Unit XP = kill XP ÷ surviving creatures; Unit Commander = 25% of the unit's kill XP − 25% of the value of its own lost figures, never below 0; Brigade Commander = 25% of each subordinate Unit Commander's award + 25% of the kill XP of subordinate skirmish/mob units; Army Commander = 50% of the brigade awards, or the brigade formula over the whole side when there are no brigades. Heroes' kills are listed to add to their normal AD&D awards. `creditKills`, `campaignResolution`. **Roster values (v0.66.2):** each roster row carries the DMG Appendix E value at the row's average hit points (d8 → 4.5, ½ HD d4 → 2.5, 1-1 → 3.5), e.g. Orc 10 + 1/hp → 15, Ogre 90 + 5/hp at 4+1 → 185, Griffon 375 + 10/hp → 690. Men-at-arms and crews are 0-level men on the DMG <1-1 row (5 + 1/hp → 10); cavalry is rider + mount with the mount on its hit-dice bracket (light 39, medium 41, heavy 95, pegasus 142, hippogriff 136, griffon 700). Chariots and CUSTOM stay blank for the sheet; a sheet value always overrides the roster.
- **[16.1] Recovery — BOARD PROCEDURE.** The rule needs to know which side holds the ground where each unit's dead lie. At ⚑ Result the GM answers once: Side A holds the field, Side B holds it, or it is split at the table's mid-depth with each side holding the half it deployed on. A surviving unit's dead lie where it stands; a destroyed or routed-off unit's dead lie where it left the board (`noteFallen`). On its own ground a side recovers ⅔ of its dead; the enemy takes ⅓ of the dead on ground it holds as prisoners; ⅔ of forces routed off the table return. Fractions round down. Nothing is applied to the board — the result is logged and downloaded as JSON for the campaign.
