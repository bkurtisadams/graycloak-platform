...
- **[14.14] Illusions — v0.68.0 first slice.** Printed illusion spells are recognized as Missile & Magic-only uses and place a persistent, described three-dimensional point marker, immediately, in a later phase, or in the next Game Round. The marker runs the printed disbelief procedure: 1:1 forces go directly to save vs Spell; 2:1/5:1/10:1 forces first make a Morale Check; a missing expected sensory component gives +1 and the referee can apply the printed ±1 plausibility adjustment. When an allied force disbelieves, a force that previously failed Morale may save in the following Game Round. **Still referee-managed:** exact figures/ranks for illusory troops, concealment, illusory terrain/hazards and real-object disbelief mistakes. `BattlesystemMagicIllusions`, `state.illusions`, `attemptIllusionDisbelief`.
