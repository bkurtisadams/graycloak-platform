...
- **[7.11] Wings follow the enemy — RULING.** Wing figures carry an anchor (enemy, side, rank) and re-seat against that enemy's *current* flank whenever its card re-shapes **or moves** — an envelopment travels with the enveloped unit. The anchor drops only when the wrapper itself moves off, the enemy is destroyed or routs, or the wrapper routs; and when it drops the figure returns to its own line rather than being left standing in open ground. v0.61.4.1, corrected v0.61.11.
