...
(function() {
  ...
  function listPick(title, items, opts) {
    ...
    // remove an item and refresh the list in place without closing.
    function renderList() {
      footer.innerHTML = '';
      if (!items.length) {
        body.innerHTML = '<div class="char-empty">' + (opts.emptyText || 'No items found.') + '</div>';
        const ok = document.createElement('button');
        ok.className = 'mp-modal-btn primary';
        ok.textContent = 'OK';
      ...
      if (opts.onDelete) {
        body.querySelectorAll('.ci-del-btn').forEach(btn => {
          btn.addEventListener('click', (e) => {
            ...
            // Inline confirm — a nested modal would clobber this picker (shared modal/resolver)
            ...
            wrap.querySelector('.ci-del-yes').addEventListener('click', async (ev) => {
              ...
              if (removed) items.splice(i, 1);  // keep caller's array in sync (passed by reference)
              renderList();
            });
