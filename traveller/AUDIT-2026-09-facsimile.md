...
| NPC combat behaviour | B1 p.32, B3 p.24 | Neither book gives a tactical procedure: Book 1 says only that an NPC escapes "at the option of the referee", and Book 3 leaves an encountered group's actions to the referee. `src/npc-tactics.js` is therefore a house routine, kept in the client rather than the rules package: engage the reachable target with the best throw; if the best throw cannot be made on 2D, close; if nothing is reachable, close on the nearest; otherwise stand. It produces a declaration, so it feeds the same intent path a referee's or player's click does | RULED — Graycloak policy, client-side |
