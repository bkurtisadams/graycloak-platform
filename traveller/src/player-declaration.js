...
export const PLAYER_DECLARATION_ACTIONS = Object.freeze(['attack', 'close', 'open', 'close-only', 'open-only', 'close-run', 'open-run', 'evade', 'wait']);
...
export function createPlayerDeclaration({ uid, actorId, action, targetId = null, round, declaredAt = Date.now() } = {}) {
  ...
  const needsTarget = ['attack', 'close', 'open', 'close-only', 'open-only', 'close-run', 'open-run'].includes(action);
...
export function authorizePlayerDeclaration(raw, { campaign, encounter } = {}) {
  ...
  if (encounter.engagement && !encounter.engagement.contactStarted) throw new Error('combat declarations are not open before contact');
  if (encounter.engagement?.moraleDue?.length) throw new Error('combat declarations are blocked by a morale check');
  ...
  if (surpriseSide && actor.side !== surpriseSide) throw new Error('this combatant is surprised and cannot act');
