...
import { renderSubsectorMap, createSvgNode, SUBSECTOR_SVG_GEOMETRY, subsectorHexCenter } from './subsector-svg.js?v=v0.336.0';
import { renderReactionPanel } from './reaction-panel.js?v=v0.336.0';
import { FAR_MERIDIAN_SUBSECTOR } from '../world/far-meridian-subsector.js?v=v0.336.0';
import { rangeBandForBandGap, ENCOUNTER_RANGE_LINE_ESCAPE_BANDS } from '../src/encounter-document.js?v=v0.336.0';
import {
...
import { renderVectorFight, renderPhaseTrack, renderDataCards } from './vector-fight-view.js?v=v0.336.0';
import { kindButton, kindIcon } from './kind-button.js?v=v0.336.0';
import { renderSectionStrip } from './section-strip.js?v=v0.336.0';
export { renderSectionStrip };
import { actorBadge, shipBadge } from './sheets.js?v=v0.336.0';
import { woundPromptFrom, initialWoundDraft, previewWoundDraft, renderWoundGroups, renderWoundPreview } from './wound-dialog.js?v=v0.336.0';
...
import { renderVectorSceneStage } from './ship-vector-map.js?v=v0.336.0';
...
function patronsPanel(p, handlers) {
  ...
  const parts = [];
  const solo = p.mode === 'game';
  const referee = p.seat !== 'player';
  // v0.322.0: jobs to be carried out here (original solo tables).
  for (const task of p.tasks ?? []) {
    parts.push(h('section', { class: 'patron-card is-task', 'aria-label': 'A job to carry out' },
      h('p', { class: 'eyebrow', text: `Job here${task.attempts ? ` \u00b7 tried ${task.attempts} time${task.attempts === 1 ? '' : 's'}` : ''}` }),
      h('h3', { text: task.title }),
      h('p', { text: task.figure }),
  ...
  if (p.patron) {
    ...
    const offer = draft ? h('dl', { class: 'pairs patron-offer' },
      h('dt', { text: 'The job' }), h('dd', { text: draft.title }),
      h('dt', { text: 'Where' }), h('dd', { text: `${draft.destinationName}${draft.distance ? `, ${draft.distance} parsec${draft.distance === 1 ? '' : 's'}` : ', here'}` }),
      h('dt', { text: 'Pay' }), h('dd', { text: cr(draft.paymentCr) }),
      h('dt', { text: 'Days' }), h('dd', { text: String(draft.deadlineDays) }),
      draft.task ? [h('dt', { text: 'What it takes' }), h('dd', { text: `${draft.task.days} days ${draft.task.where === 'surface' ? 'searching' : 'asking'} at ${draft.destinationName}, then 2D + ${draft.task.skills.join(', ')} for ${draft.task.needed}+` })] : null,
      draft.kind === 'smuggling' ? [h('dt', { text: 'The risk' }), h('dd', { text: `${draft.cargoTons} t in the hold; the law throw on landing (2D, the law level or more), jail if caught` })] : null) : null;
    const form = !solo && referee ? h('div', { class: 'patron-form' },
      h('label', {}, h('span', { text: 'The job' }), h('input', { type: 'text', 'data-title': true, placeholder: 'What the patron wants done', value: draft?.title ?? '' })),
      h('label', {}, h('span', { text: 'Where' }), h('select', { 'data-destination': true }, [...p.systems].sort((a, b) => a.name.localeCompare(b.name)).map((entry) => h('option', { value: entry.id, selected: entry.id === (draft?.destinationSystemId ?? patron.systemId), text: entry.name })))),
      h('label', {}, h('span', { text: 'Pay (Cr)' }), h('input', { type: 'number', min: 0, step: 500, value: draft?.paymentCr ?? 0, 'data-pay': true })),
      h('label', {}, h('span', { text: 'Days' }), h('input', { type: 'number', min: 1, value: draft?.deadlineDays ?? 30, 'data-days': true })),
      h('label', { class: 'is-wide' }, h('span', { text: 'Terms' }), h('input', { type: 'text', 'data-notes': true, placeholder: 'Expenses, shares, what the patron keeps' }))) : null;
    const take = () => people('patrons:accept', form ? {
    ...
    parts.push(h('section', { class: 'patron-card', 'aria-label': 'A patron' },
      ...
      solo ? h('p', { class: 'cite', text: 'Solo: the game says what the patron wants (original tables, not the book\u2019s).' })
        : referee ? h('p', { class: 'cite', text: 'Referee: build the patron as an NPC and decide the mission, or have one suggested, and write it up below (The Traveller Book p.99).' })
          : h('p', { class: 'cite', text: `The referee is deciding what the ${patron.type.toLowerCase()} wants.` }),
      solo || !referee ? offer : null,
      form,
      waiting ? null : h('div', { class: 'lead-actions' },
        !solo && referee ? kindButton({ label: 'Suggest a job', kind: 'optional' }, { small: true, onclick: () => people('patrons:suggest') }) : null,
...
function worldCaption(system, { role, label, world = null, note = null } = {}) {
  // v0.321.0: a world the players have not visited shows its chart facts only.
  if (system.hidden) {
    return h('div', { class: `caption caption-${role}` },
      h('p', { class: 'caption-role', text: label }),
      h('h2', {}, system.name, h('span', { class: 'code', text: ` ${system.hex}` })),
      h('p', { class: 'caption-facts', text: [`Starport ${system.mainWorld.uwp[0]}`, system.bases?.naval ? 'Naval base' : null, system.bases?.scout ? 'Scout base' : null, system.gasGiant ? 'Gas giant' : null, system.travelZone && system.travelZone !== 'none' ? `${system.travelZone} zone` : null].filter(Boolean).join(' \u00b7 ') }),
      h('p', { class: 'cite', text: 'Chart facts only: the world\u2019s full profile is learned on arrival.' }),
...
export function subsectorScene(scene, { onSelectSystem, onCommand, onExportSector = null }, readOnly = false) {
  // v0.321.0: the campaign's map from the session — the sector as charted,
  // in a frame round the charted subsectors — or Far Meridian for samples.
  const subsector = scene.map ?? FAR_MERIDIAN_SUBSECTOR;
  const frame = scene.map?.frame ?? { firstColumn: 1, firstRow: 1, columns: SUBSECTOR_COLUMNS, rows: SUBSECTOR_ROWS };
  let current;
...
function playersPanel(model, state, handlers) {
  ...
  const travellers = (state.travellers ?? []).map((traveller) => {
    ...
    return h('li', { class: 'entry traveller-row' },
      ...
      // v0.336.0: a mustering-out ship rolled but not yet built.
      state.live && traveller.shipBenefit ? h('button', { type: 'button', class: 'button is-small is-primary traveller-ship', text: `Bring in their ${traveller.shipBenefit}`,
        title: traveller.shipBenefit === 'Free Trader' ? 'Book 1 pp.22-23: a Type A they own, with the mortgage from today' : 'Book 1 p.23: a Type S on reserve from the Scout Service',
        onclick: () => command(`ship:from-benefit:${traveller.id}`) }) : null,
...
function settingsDrawer(state, handlers) {
  ...
  return [
    ...
    state.live && handlers.onCommand ? h('fieldset', { class: 'setting referee-tools' },
      ...
      kindButton({ label: 'Clear waiting encounters', kind: 'neutral' }, { small: true, onclick: () => handlers.onCommand('referee:clear-encounters') }),
      kindButton({ label: 'Put the passengers ashore here', kind: 'neutral' }, { small: true, onclick: () => { if (globalThis.confirm?.('Put every passenger aboard ashore here? Their fares are not paid.') ?? true) handlers.onCommand('referee:passengers-ashore'); } })) : null
  ];
}
