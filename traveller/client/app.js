import {
  ...
  COMPUTER_PROGRAMS
} from '../vendor/classic-traveller-rules/index.js?v=v0.207.1';
...
import {
  ...
  nobleTitleLabel,
  serviceName
} from './ui-model.js?v=v0.207.1';

import {
  TRAVELLER_DOCUMENT_KINDS,
  loadTravellerDocument
} from './document-loader.js?v=v0.207.1';

import { createTravellerInvite, generateInviteCode, unassignedWorld, importCharacterRecord, WORLD_KINDS } from '../src/character-record.js?v=v0.207.1';
import { createCampaignHome, nextCampaignHome, importCampaignHome, campaignHomeBytes, StaleCampaignHomeError, CAMPAIGN_HOME_SOFT_LIMIT_BYTES } from '../src/campaign-home.js?v=v0.207.1';
import { resolveShipImport } from '../src/ship-import.js?v=v0.207.1';
import { stagingTokenMenuModel, fightTokenMenuModel, launcherBlockedReason, vectorToward, baseShipLabel } from '../src/ship-token-menu.js?v=v0.207.1';
import { dataCardLines } from '../src/ship-data-card-text.js?v=v0.207.1';
import { spaceSceneCombatPlan, spaceSceneLink, writeSpaceCombatToScene } from '../src/space-scene-combat.js?v=v0.207.1';
import { setSceneTokenSide, setSceneTokenLabel, defaultShipVector, SCENE_VECTOR_DEFAULT_SPEED } from '../src/scene-document.js?v=v0.207.1';
import { createSceneDocument, updateSceneDocument, sceneFolders, sceneBoardMeters, sceneBoardCells, placeSceneToken, moveSceneToken, removeSceneToken, placeSceneShip, moveSceneShip, setSceneShipVector, sceneGravityWorld, sceneBodies, placeSceneBody, moveSceneBody, removeSceneBody, worldBody, asteroidFieldBody, emplacementBody, SCENE_WORLD_MAX_DIAMETER, trackedSceneTokens, SCENE_MIN_SQUARES, SCENE_MAX_METERS, SCENE_VECTOR_DEFAULT_SPAN, SCENE_VECTOR_MIN_SPAN, SCENE_VECTOR_MAX_SPAN, sceneIsVectorBoard, duplicateSceneDocument, moveScenesToFolder, adoptSceneDocument, sceneThumbnailSvg, sceneMatchesSearch, sceneActorIsDesignReference, exportSceneDocument, importSceneDocument, DEFAULT_SCENE_FOLDER } from '../src/scene-document.js?v=v0.207.1';
import { directoryFolders, removeEncounterFromCampaign
} from '../src/campaign-document.js?v=v0.207.1';
import { createSceneCanvas, svgNode as sceneSvgNode } from './scene-canvas.js?v=v0.207.1';
...
import { renderShipVectorMap, renderVectorSceneStage, vectorSelectedShipId, vectorHoveredShipId } from './ship-vector-map.js?v=v0.207.1';
import { targetHoveredShip, clearShipTargets, targetsOfShip } from '../src/ship-targeting.js?v=v0.207.1';
import {
...
} from '../src/document-window.js?v=v0.207.1';
import { TRAY_DICE, rollFormula, formatRoll, createChatMessage, interpretChatInput, parseRollFormula } from '../src/dice-tray.js?v=v0.207.1';
import { inspectElement, formatInspection } from '../src/ui-debug.js?v=v0.207.1';
...
} from './chargen-view.js?v=v0.207.1';

import {
  generateCharacterName,
  generateShipName,
  generateShipRegistry
} from './generators.js?v=v0.207.1';
...
import {
  ...
  subsectorHexCenter,
  subsectorSvgViewBox
} from './subsector-svg.js?v=v0.207.1';
...
import {
  ...
  weeklyTradeSeed,
  saleQuoteSeed
} from './commerce-market.js?v=v0.207.1';
...
import {
  ...
  clearNpcActorConditions, duplicateNpcActorDocument, setNpcActorArchived, npcActorMatchesSearch, exportNpcActorDocument } from '../src/npc-actor-document.js?v=v0.207.1';
import { synchronizeEncounterDocuments } from '../src/combatant-document-sync.js?v=v0.207.1';
import { chooseNpcDeclaration, pendingNpcDeclarations } from '../src/npc-tactics.js?v=v0.207.1';
import { initAuth, onAuthChange, signOutOfTraveller, currentUserId, authStatus } from './auth.js?v=v0.207.1';
import { openSignInDialog } from './signin-ui.js?v=v0.207.1';
import { publishCampaign, publishEncounterView, publishStatus, seatPlayer, unseatPlayer, listSeatedPlayers, watchDeclarations, clearDeclarations, clearChat, watchWoundAllocations, clearWoundAllocations, watchTokenMoves, clearTokenMove, watchCanvasPresence, publishPlayerCharacter, removePlayerCharacter, publishPlayerLog, createInvite, deleteInvite, listCampaignInvites, watchJoinRequests, deleteJoinRequest, setCharacterRecordWorldRemote, saveCampaignHome, loadCampaignHome, loadCharacterRecord, sendChatMessage, watchChat } from './publish.js?v=v0.207.1';
import { authorizePlayerDeclaration } from '../src/player-declaration.js?v=v0.207.1';
import { authorizePlayerTokenMove, playerMoveToCombatantMove, authorizePlayerSceneMove } from '../src/player-token-movement.js?v=v0.207.1';
import { buildPublishedView, buildPublishedCampaign, buildPublishedCharacter, buildPublishedLog, buildPublishedScene } from '../src/published-view.js?v=v0.207.1';
import { createMediaAssetDocument, importMediaAssetDocument } from '../src/media-asset-document.js?v=v0.207.1';
import {
  ACTIVITY_VISIBILITY,
...
} from '../src/activity-log-document.js?v=v0.207.1';
...
} from '../src/player-session.js?v=v0.207.1';
...
import {
  ...
  normalizeQuickSlots,
  resolveQuickSlots
} from './quick-slots.js?v=v0.207.1';

import {
  exportCampaignBundle
} from '../src/campaign-bundle.js?v=v0.207.1';

import {
  createDocumentRegistry
} from '../src/document-registry.js?v=v0.207.1';

import {
  createActivityLogStore
} from '../src/activity-log.js?v=v0.207.1';
...
} from '../src/contract-document.js?v=v0.207.1';
...
} from '../src/situation-document.js?v=v0.207.1';
...
import {
  ...
  pendingWoundAllocation, allocateRoundWound, ESCAPE_TARGET, ESCAPE_RANGE_DMS } from '../src/encounter-document.js?v=v0.207.1';
// v0.178.0: the throw card, the two-step declaration and the attack result
// card are pure view models; app.js draws them.
import { authorizePlayerWoundAllocation } from '../src/player-wound-allocation.js?v=v0.207.1';
import { woundPromptFrom, initialWoundDraft, previewWoundDraft, renderWoundGroups, renderWoundPreview, woundHitLine } from './wound-dialog.js?v=v0.207.1';
import { throwCardModel, deriveDeclaration, declarationSummary, attackCardModel, signed as signedDMText, rangeLabel, woundFormula } from './combat-view.js?v=v0.207.1';

import {
  createContactDocument,
  importContactDocument,
  touchContactDocument
} from '../src/contact-document.js?v=v0.207.1';

import {
  importAdventureThreadDocument,
  linkAdventureThreadDocument
} from '../src/adventure-thread-document.js?v=v0.207.1';
...
} from '../world/situation-events.js?v=v0.207.1';

import {
  generateContractBoard
} from '../world/contract-board.js?v=v0.207.1';

import {
  applySituationThreadConsequences
} from '../world/thread-consequences.js?v=v0.207.1';

import {
  FAR_MERIDIAN_SUBSECTOR
} from '../world/far-meridian-subsector.js?v=v0.207.1';
...
function renderRangeLineBoard(encounter) {
  ...
  const BANDS = ENCOUNTER_RANGE_LINE_ESCAPE_BANDS + 1; // 0..20 and the escape band
  const width = 1206;
  const height = 1206;
  const marginX = 18;
  const bandTop = 150;
  ...
  // v0.206.0: the 1981 band table (ruling in src/encounter-document.js). Close is contact inside a band, so a gap alone reads short.
  const zoneForGap = (gap) => gap === 0 ? 'short' : gap <= 2 ? 'medium' : gap <= 10 ? 'long' : gap <= 20 ? 'very-long' : 'escaped';
  const zoneLabel = { close: 'CLOSE', short: 'SHORT', medium: 'MEDIUM \u00b7 1\u20132', long: 'LONG \u00b7 3\u201310', 'very-long': 'VERY LONG \u00b7 11\u201320', escaped: 'ESCAPED' };
  ...
  if (actor && target && actor.id !== target.id) {
    ...
    // Contact decides close against short inside a band, so ask the engine.
    const pairZone = gap > 20 ? 'escaped' : encounterPairRange(actor, target, 'range-line');
    pairLabel.textContent = `${zoneLabel[pairZone].split(' \u00b7 ')[0]} \u00b7 ${gap} BAND${gap === 1 ? '' : 'S'} \u00b7 ${gap * 25} M`;
