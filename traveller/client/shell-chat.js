// v0.196.0: chat is the shell's bottom edge. Closed, it is the composer and
...
function setOpen(bar, open, { persist = true } = {}) {
  ...
  if (open) {
    const feed = bar.querySelector('[data-chat-feed], #activity-feed');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  bar.querySelector('[data-chat-input], #chat-input')?.addEventListener('focus', () => { if (!bar.classList.contains('open')) setOpen(bar, true); });
