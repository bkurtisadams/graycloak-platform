...
import {
  ...
  writeWoundAllocation } from './publish.js?v=v0.335.0';
import { createPlayerDeclaration } from '../src/player-declaration.js?v=v0.335.0';
import { createPlayerWoundAllocation } from '../src/player-wound-allocation.js?v=v0.335.0';
...
import { deriveDeclaration, declarationSummary, rangeLabel, signed as signedDM, woundFormula } from './combat-view.js?v=v0.335.0';
import { createPlayerTokenMove } from '../src/player-token-movement.js?v=v0.335.0';
import { serviceName, nobleTitleLabel, buildServiceHistory, buildGenerationLog } from './ui-model.js?v=v0.335.0';
import { PERSONAL_WEAPONS, SUBSECTOR_COLUMNS, SUBSECTOR_ROWS, getSubsectorSystem } from '../vendor/classic-traveller-rules/index.js?v=r0.82.0';
import { renderSubsectorMap } from './subsector-svg.js?v=v0.335.0';
import { createSceneCanvas, svgNode } from './scene-canvas.js?v=v0.335.0';
import { renderVectorSceneStage } from './ship-vector-map.js?v=v0.335.0';
import { publishedVectorSceneDocument } from '../src/published-view.js?v=v0.335.0';
...
async function connect(campaignId) {
  try {
    ...
    unsubscribeCampaign = db.doc(`travellerCampaigns/${campaignId}`).onSnapshot(
      (snapshot) => {
        if (!snapshot.exists) {
          setStatus('NO SUCH CAMPAIGN, OR YOU ARE NOT A PLAYER IN IT', 'error');
          return;
        }
      ...
      (error) => setStatus(`${error.code === 'permission-denied' ? 'NOT A PLAYER IN THIS CAMPAIGN' : error.message}`, 'error')
    );
  } catch (error) {
    setStatus(error?.message ?? String(error), 'error');
  }
