...
test('v0.15.2 establishes a Traveller-first campaign hierarchy and removes duplicated play text', async () => {
  ...
  assert.match(html, /id="app-title" class="title">TRAVELLER</);
  assert.match(html, /id="app-subtitle" class="subtitle">v0\.97\.0</);
...
test('v0.49.0 offers NPC declarations as advice and as automation', async () => {
  ...
  assert.match(app, /function autoFillActiveEncounterNpcs\(\)/);
  assert.match(app, /AUTO-FILL NPCs/);
  assert.doesNotMatch(app, /const active = applyNpcDeclarations\(started\);/);
...
test('v0.54.0 publishes the scene automatically when a round resolves', async () => {
  ...
  // the fight.
  assert.ok(app.match(/autoPublishEncounterView\(result\.encounter\);/g).length >= 2);
...
test('v0.59.0 lets players declare and the referee apply those declarations', async () => {
  ...
  assert.match(view, /declaringRound: declarationsOpen \? encounter\.round : null/);
...
test('v0.60.0 rechecks player ownership and retires stale scene listeners', async () => {
  ...
  assert.match(index, /v0\.97\.0/);
  assert.match(playerPage, /PLAYER v0\.97\.0/);
});
...
test('v0.62.0 exposes Book 1 walking, running, escape limits, and shared movement paths', async () => {
  ...
  assert.match(encounter, /export function attemptEncounterEscape/);
  assert.match(encounter, /escape is attempted before combat or contact begins/);
  assert.match(app, /RUN CLOSER/);
  assert.match(player, /\['close-run', 'RUN CLOSE'\]/);
  assert.doesNotMatch(player, /\['ESCAPE', 'escape'/);
...
test('v0.67.0 gives players a front door: their own characters, chargen, and a seat by invite', async () => {
  ...
  assert.match(html, /ENTER v0\.97\.0/);
...
test('v0.94.0 the encounter has a setup phase, and the tracker has one scrollbar', async () => {
  ...
  assert.match(doc, /export function beginEncounter\(document, \{ surpriseConditions = \{\}, contactStarted = true, dice \} = \{\}\)/);
