...
test('v0.14.0 expands combat into a movable multi-party range-guided workspace', async () => {
  ...
  assert.match(html, /201 by 201 square personal encounter map/);
  ...
  assert.match(encounter, /ENCOUNTER_MAP_COLUMNS = 201/);
...
test('v0.15.2 establishes a Traveller-first campaign hierarchy and removes duplicated play text', async () => {
  ...
  assert.match(html, /id="app-title" class="title">TRAVELLER</);
  assert.match(html, /id="app-subtitle" class="subtitle">v0\.62\.1</);
...
test('v0.51.0 adds sign-in without making the client depend on it', async () => {
  ...
  assert.match(app, /initAuth\(\)\.then\(\(\) => render\(\)\);/);
...
test('v0.52.0 publishes a player-safe copy without making the client depend on it', async () => {
  ...
  // never fetches it.
  assert.match(publish, /export async function ensureFirestore\(\)/);
...
test('v0.53.0 keeps the publish controls reachable and remembers being online', async () => {
  ...
  assert.ok(
    app.indexOf('await publishCampaign(published);') < app.indexOf('campaignDocument = markCampaignPublished'),
...
test('v0.55.0 seats players and lets them read their own account id', async () => {
  ...
  assert.match(app, /if \(owner === uid\) campaignDocument = setDocumentOwner\(campaignDocument, \{ documentId, ownerUid: '' \}\);/);
...
test('v0.60.0 rechecks player ownership and retires stale scene listeners', async () => {
  ...
  assert.match(index, /v0\.62\.1/);
  assert.match(playerPage, /PLAYER v0\.62\.1/);
});

test('v0.60.3 uses a five-meter grid, compact shared-square tokens, and selected-actor range boundaries', async () => {
  const html = await read('index.html');
  const app = await read('app.js');
  const css = await read('styles.css');

  assert.match(html, /201 by 201 square personal encounter map; each square is 5 meters/);
...
test('v0.62.0 exposes Book 1 walking, running, escape limits, and shared movement paths', async () => {
  ...
  assert.match(encounter, /ENCOUNTER_SQUARES_PER_RANGE_BAND = 5/);
...
test('v0.62.1 previews a snapped drag with one dash pattern and legality colors', async () => {
  ...
  assert.match(player, /distance \* 5/);
