...
test('v0.15.2 establishes a Traveller-first campaign hierarchy and removes duplicated play text', async () => {
  ...
  assert.match(html, /id="app-title" class="title">TRAVELLER</);
  assert.match(html, /id="app-subtitle" class="subtitle">v0\.39\.0</);
...
test('v0.16.0 adds live roster placement, body-aware conditions, typed tokens, and explicit range guidance', async () => {
  ...
  assert.match(app, /RANGE \$\{guide\.suggestedRange/);
...
test('v0.36.0 puts the encounter map in the centre scene and the DM panel and rosters in the rail', async () => {
  ...
  for (const id of ['encounter-dm-panel', 'encounter-actions', 'encounter-party-roster', 'encounter-roster', 'encounter-record']) {
    assert.ok(
  ...
  assert.match(app, /previewPersonalAttack\(\{ attacker: actor, defender: target, range: band \}\)/);
...
test('v0.39.0 lets the referee declare for any side in the UI', async () => {
  ...
  assert.match(app, /DECLARE FOR THIS ACTOR/);
  assert.match(app, /if \(event\.shiftKey\) setEncounterActor\(encounter\.identity\.id, enemy\.id\);/);
  assert.match(app, /ORDERED \$\{ordered\.action\.toUpperCase\(\)\}/);

  // The roster covers every non-party side, not just 'opposition'.
  assert.match(app, /encounter\.combatants\.filter\(\(entry\) => entry\.side !== 'party'\)\.map\(\(enemy\)/);
});
