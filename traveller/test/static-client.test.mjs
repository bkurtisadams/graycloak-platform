...
test('v0.15.2 establishes a Traveller-first campaign hierarchy and removes duplicated play text', async () => {
  ...
  assert.match(html, /id="app-title" class="title">TRAVELLER</);
  assert.match(html, /id="app-subtitle" class="subtitle">v0\.181\.0</);
...
test('v0.31.0 gives the centre three scene tabs, a status strip, and viewport-height columns', async () => {
  ...
  for (const id of ['map-zoom-out', 'map-zoom-in', 'map-zoom-fit', 'map-zoom-label', 'subsector-legend',
    'jump-actions', 'toggle-system-details']) {
...
test('v0.60.0 rechecks player ownership and retires stale scene listeners', async () => {
  ...
  assert.match(index, /v0\.181\.0/);
  assert.match(playerPage, /PLAYER v0\.181\.0/);
});
...
test('v0.67.0 gives players a front door: their own characters, chargen, and a seat by invite', async () => {
  ...
  assert.match(html, /ENTER v0\.181\.0/);
...
test('v0.76.1 picking a sidebar tab closes the character strip so the tab is not hidden below it', async () => {
  const app = await read('app.js');
  assert.match(app, /const changed = tab !== sidebarTab;/);
  // v0.98.0: WHAT NOW? is no longer one of these strips, so only the
  // character strip closes with a tab choice.
  assert.match(app, /if \(changed\) \{\n    const character = document\.querySelector\('#sidebar-character'\);/);
  assert.match(app, /if \(character\) character\.open = false;/);
});
...
test('v0.96.8 how position is tracked is the first, unavoidable choice in Manual Combat', async () => {
  ...
  assert.match(html, /<input type="radio" name="combat-board-mode" value="scene" checked>/);
  assert.match(html, /<input type="radio" name="combat-board-mode" value="range-line">/);
...
test('v0.120.0 the world record stacks alone; trade and jobs left for the dock', async () => {
  const app = await read('app.js');
  const css = await read('styles.css');
  const html = await read('index.html');
  ...
  // TRADE and JOBS are flyouts, so only the world record stacks here now.
  assert.match(app, /const stacked = portStacked && key === 'port';/);
...
test('v0.106.0 the ship reads above the map, and a fight hides it', async () => {
  const app = await read('app.js');
  const html = await read('index.html');
  const css = await read('styles.css');
  ...
  assert.ok(section.indexOf('id="ship-strip"') < section.indexOf('class="subsector-map-tools"'));
...
test('v0.115.0 state tags read as a traffic light; kind tags stay neutral', async () => {
  ...
  assert.match(css, /\.procedure-card\.blocked \.procedure-card-tag \{ color: #8c3b30; \}/);
  assert.match(css, /\.procedure-card\.required \.procedure-card-tag \{ color: #6b5f1f; \}/);
...
test('v0.129.0 the jump action sits under its own readout and the top band is gone', async () => {
  ...
  const world = html.slice(html.indexOf('nav-strip-world'), html.indexOf('subsector-map-tools'));
...
test('v0.134.0 phase E can change the computer, and the CPU choice is the player\'s', async () => {
  ...
  assert.match(app, /const wouldOverflow = !loaded && state\.loadedSpace \+ program\.space > state\.inComputerCapacity;/);
});
...
test('v0.137.0 a boarding hands off to a Book 1 fight on the range-line board', async () => {
  ...
  assert.match(app, /if \(!assessment\.allowed\) continue;/);
  assert.match(app, /BOARD \$\{target\.name\.toUpperCase\(\)\}/);
...
test('v0.149.0 combat actions are reachable and the map is not lost to a fight', async () => {
  ...
  assert.match(app, /el\.shipCombatActions\.append\(heading\);/);
...
test('v0.152.0 gives the reload lock and the rack display a rail control', async () => {
  ...
  assert.match(app, /if \(turretReloadLock\(participant, turretId\)\)/);
...
test('the paused wound is published to the player and answered from their page', async () => {
  ...
  assert.match(app, /const ownerUid = pending\?\.defender \? seatedOwnerOfCombatant\(pending\.defender\.id\) : null;/);
...
test('a fight widens the drawer and splits the panel in two columns', async () => {
  const css = await readFile(new URL('../client/styles.css', import.meta.url), 'utf8');
  // combat-focus already says when a fight holds the rail; nothing new decides it.
  assert.match(css, /\.terminal\.shell\.combat-focus \.shell-sidebar \{ grid-template-columns: minmax\(0, 620px\) 52px; \}/);
  // The v0.80.2 ruling stands: the drawer pushes the stage, never covers it.
  assert.match(css, /\.terminal\.shell\.combat-focus:not\(\.sidebar-collapsed\) \.shell-stage \{ padding-right: calc\(620px \+ 52px\); \}/);
  ...
  assert.match(css, /@media \(min-width: 1200px\) \{\s*\.combat-focus \.sidebar-panel\[data-sidebar-panel="combat"\] > \.combat-rail \{/);
  assert.match(css, /\.combat-focus \.combat-rail > #encounter-tracker \{\s*grid-column: 1; grid-row: 2 \/ -1;/);
  assert.match(css, /\.combat-focus \.combat-rail > #encounter-resolve \{ grid-column: 2; grid-row: 3;/);
  assert.doesNotMatch(css, /box-shadow/);
});
...
test('the board carries the wound track and the needed throw', async () => {
  ...
  assert.match(app, /appendTokenWoundPips\(group, combatant\);\s*appendTokenThrowBadge\(group, encounter, selectedEncounterActor\(encounter\), combatant\);/);
