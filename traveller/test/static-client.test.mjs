...
test('v0.15.2 establishes a Traveller-first campaign hierarchy and removes duplicated play text', async () => {
  ...
  assert.match(html, /id="app-title" class="title">TRAVELLER</);
  assert.match(html, /id="app-subtitle" class="subtitle">v0\.338\.0</);
...
test('v0.60.0 rechecks player ownership and retires stale scene listeners', async () => {
  ...
  assert.match(index, /v0\.338\.0/);
  assert.match(playerPage, /PLAYER v0\.338\.0/);
});
...
test('v0.67.0 gives players a front door: their own characters, chargen, and a seat by invite', async () => {
  ...
  assert.match(html, /LOBBY v0\.338\.0/, 'v0.337.0: the lobby');
  ...
  assert.match(enter, /createCharacterRecord\(createCharacterDocument\(named\), \{ ownerUid: uid, role \}\)/);
  assert.match(enter, /YOU HAVE NO CHARACTERS YET/);
  assert.match(enter, /\[ OPEN \]/, 'v0.289.0: a campaign card opens with OPEN');
  assert.match(enter, /seat\.html\?campaign=/, 'v0.284.0: the lobby opens the player seat');
  assert.match(enter, /SOLO WORLD \(NOT YET OPEN\)/, 'v0.289.0: said on the row');
...
test('v0.70.0 shows the player the world between fights and publishes an active board on every save', async () => {
  ...
  assert.match(svg, /export function renderSubsectorMap\(\{ subsector, columns, rows, current = null, selected = null, reachable = new Map\(\), objectives = new Set\(\), onSelect = null, lanes = \[\], offLane = new Set\(\), firstColumn = 1, firstRow = 1, borders = \[\], worldStyle = 'classic', edges = \[\] \} = \{\}\)/);
...
test('v0.94.0 the encounter has a setup phase, and the tracker has one scrollbar', async () => {
  ...
  assert.match(doc, /export function beginEncounter\(document, \{ surpriseConditions = \{\}, dice, surprise = 'roll', thrown = null \} = \{\}\)/);
  // Surprise belongs to the start of the fight, not to creation.
  assert.match(doc, /next\.surprise = carried \?\? \(surprise === 'roll'\n {4}\? \{\n {6}\.\.\.resolvePersonalSurprise\(\{/);
...
test('v0.109.0 a stale vendored rules package reports itself instead of a bare SyntaxError', async () => {
  ...
  // v0.327.0: the loaded package is asked its version before app.js loads.
  assert.match(boot, /const rules = await import\(RULES\);\n  if \(rules\.RULES_VERSION !== EXPECTED_RULES_VERSION\)/);
...
test('v0.127.0 a change the player made reaches the campaign home, not just the cache', async () => {
  ...
  //   takeFreeTrader         — same (v0.333.0), the Merchant's ship benefit
  const allowed = new Set([
    'persistCampaignState', 'saveCampaignHomeNow', 'saveCampaignLocal',
    'exportCampaignPortable', 'newCampaign', 'assignScoutShip', 'takeFreeTrader'
  ]);
  const offenders = [];
  let fn = null;
...
test('v0.145.0 a load failure names the right fault and the sync checks exports', async () => {
  ...
  assert.match(boot, /function describe\(message, kind = null\)/);
...
test('v0.146.0 every module the browser fetches carries the client version', async () => {
  ...
  const rules = JSON.parse(await readFile(new URL('../vendor/classic-traveller-rules/package.json', import.meta.url), 'utf8'));
  const rulesStamp = `r${rules.version}`;
  ...
  for (const file of await readdir(dir)) {
    ...
    for (const pattern of patterns) {
      for (const [, specifier, query, version] of text.matchAll(pattern)) {
        // v0.327.0: the rules package carries its own stamp, r<version>.
        const wanted = specifier.includes('/vendor/classic-traveller-rules/') ? rulesStamp : stamp;
        if (!query) unstamped.push(`${file}: ${specifier}`);
        else if (version !== wanted) wrongVersion.push(`${file}: ${specifier} is ${version}, not ${wanted}`);
      }
    }
  }
  ...
  assert.match(boot, /index\.js\?v=r\$\{EXPECTED_RULES_VERSION\}/);
  assert.match(boot, new RegExp(`const EXPECTED_RULES_VERSION = '${rules.version.replace(/\./g, '\\.')}'`));
...
test('v0.273.0 every lobby character can be deleted; a seated one can leave; deleting a campaign releases its seats', async () => {
  const enter = await read('enter.js');
  const publish = await read('publish.js');
  assert.match(enter, /menuItem\(`Leave \$\{record\.world\.campaignName \?\? 'the campaign'\}`/, 'v0.284.0: LEAVE (v0.337.0: in the character\u2019s menu)');
  assert.match(enter, /joinWith\(record, inviteFromUrl, joinInvite\.invite\)/, 'v0.289.0: joined from the link, one step');
  assert.doesNotMatch(enter, /JOIN A TABLE|SIT DOWN/, 'Kurt: campaign, not table');
  // v0.335.0: after Make NPC / Make playable, where the row is free.
  assert.match(enter, /menuItem\('Delete\\u2026', \(\) => removeRecord\(record\), \{ danger: true \}\)/, 'v0.289.0: every character row offers Delete (v0.337.0: in its menu)');
...
test('v0.277.0 the lobby enters the new player seat, which reads only what is published for the player', async () => {
  ...
  assert.match(enter, /`seat\.html\?campaign=\$\{encodeURIComponent/);
  assert.doesNotMatch(enter, /\[ OLD PAGE \]/, 'v0.284.0: the old page is retired from the lobby');
...
// v0.281.0: Clear remembers an ISO time, and the player's page sorts its chat by time.
test('v0.281.0 the referee\u2019s Clear keeps an ISO time; the player\u2019s page orders chat by time and draws the subsector', async () => {
  const play = await read('play.js');
  const seat = await read('seat.js');
  assert.match(play, /ui\.chatClearedAt = new Date\(last\)\.toISOString\(\)/);
  assert.match(seat, /sort\(\(a, b\) => Date\.parse\(a\.at\) - Date\.parse\(b\.at\)\)/);
  assert.match(seat, /subsectorScene\(\{/);
});
// v0.282.0: the player's page follows the referee's Clear and watches ship fights.
test('v0.282.0 the player page follows Clear and draws a watched ship fight', async () => {
  const seat = await read('seat.js');
  const play = await read('play.js');
  const vector = await read('vector-fight-view.js');
  assert.match(play, /source\.session\.run\('chat:clear'/);
  assert.match(seat, /state\.envelope\?\.chatClearedAt/);
  // v0.329.0: the game refereeing, the fight's buttons send requests.
  assert.match(seat, /shipFightScene\(envelope\.shipFight, \{ onCommand: \(command\) => sendRequest\(command\) \}\)/);
  assert.match(vector, /if \(shipFight\.readOnly\) \{/);
});
// v0.283.0: the player's order carries a weapon; the fight's story goes to chat.
test('v0.283.0 the player page offers a weapon in the order and tells the fight in chat', async () => {
  const seat = await read('seat.js');
  assert.match(seat, /aria-label': `\$\{fighter\.name\}: weapon`/);
  assert.match(seat, /weaponKey: draft\.weaponKey && draft\.weaponKey !== fighter\?\.weaponKey \? draft\.weaponKey : null/);
  assert.match(seat, /category: 'COMBAT'/);
  assert.doesNotMatch(seat, /'What happened'/);
});
// v0.284.0: the lobby's join link, the campaigns you play in, and the join panel.
test('v0.284.0 players join by a link that asks which character; the lobby lists campaigns you play in', async () => {
  const enter = await read('enter.js');
  const html = await read('enter.html');
  const play = await read('play.js');
  assert.match(html, /<h2>CAMPAIGNS<\/h2>/, 'v0.289.0: one list (v0.337.0: a card grid)');
  assert.doesNotMatch(html, /CAMPAIGNS YOU PLAY IN/);
  assert.match(html, /id="enter-join-panel"/);
  assert.match(enter, /searchParams\.get\('join'\)/);
  assert.match(enter, /\[ JOIN WITH \$\{record\.name\.toUpperCase\(\)\} \]/);
  assert.match(enter, /menuItem\('Copy join link', \(\) => copyJoinLink\(campaign\)/);
  assert.match(enter, /url\.search = `\?join=\$\{encodeURIComponent\(code\)\}`/);
  assert.match(play, /url\.search = `\?join=\$\{encodeURIComponent\(invite\.code\)\}`/);
});
// v0.285.0: Remove and a real Leave.
test('v0.285.0 the referee removes with the character going home; a player\u2019s Leave takes their seat too', async () => {
  const play = await read('play.js');
  const enter = await read('enter.js');
  const seat = await read('seat.js');
  const views = await read('play-views.js');
  assert.match(views, /function playersPanel\(model, state, handlers\)/);
  assert.match(play, /cloud\.releaseSeat\(campaignId, seat\.uid\)/);
  assert.match(play, /'character:release'/);
  assert.match(enter, /await leaveSeat\(campaignId, currentUserId\(\)\)/);
  assert.match(enter, /function welcomeHome\(\)/);
  assert.match(seat, /touchSeat\(campaignId, currentUserId\(\)\)/);
  assert.match(seat, /lastCampaign: next/);
});
// v0.289.0: the simpler lobby and one-step join.
test('v0.289.0 one campaigns list with OPEN and a menu; joining seats the player and goes straight to the campaign', async () => {
  const enter = await read('enter.js');
  const publish = await read('publish.js');
  const play = await read('play.js');
  assert.match(enter, /function refereeCard\(campaign\)/);
  assert.match(enter, /function playerCard\(record, waiting\)/);
  assert.match(enter, /\[ \+ NEW CAMPAIGN \]/);
  assert.match(enter, /await seatSelf\(invite\.campaignId, uid, \{ name: playerName, code \}\)/);
  assert.match(enter, /window\.location\.href = `seat\.html\?campaign=/);
  assert.match(publish, /export async function seatSelf\(campaignId, uid/);
  assert.match(play, /if \(!source\.session\.resolved\.campaign\.roster\?\.approvePlayers\)/);
});
// v0.291.0: joins are watched whenever the page is live and signed in, not
// only if both were true at the moment it started.
test('v0.291.0 the referee page watches joins from its multiplayer sync, and retries a failed bring-in', async () => {
  const play = await read('play.js');
  const sync = play.slice(play.indexOf('function syncMultiplayer()'));
  assert.match(sync, /cloud\.watchJoins\(campaignId, onJoins\)/);
  assert.match(play, /if \(ok === false\) autoAdmitted\.delete\(join\.uid\)/);
  assert.equal((play.match(/cloud\.watchJoins\(/g) ?? []).length, 1, 'one place starts it');
});
// v0.293.0: a join note survives until the character is saved in the campaign,
// and the player's page sends a lost one again.
test('v0.293.0 Admit clears the join only after the campaign save; the player page re-sends a lost join', async () => {
  const play = await read('play.js');
  const seat = await read('seat.js');
  const admit = play.slice(play.indexOf("} else if (action === 'admit') {"), play.indexOf("} else if (action === 'decline') {"));
  assert.ok(admit.indexOf('await flushSave()') > 0 && admit.indexOf('await flushSave()') < admit.indexOf('cloud.dismissJoin'), 'save, then clear');
  assert.match(seat, /async function resendJoins\(\)/);
  assert.match(seat, /function ownedHere\(\)/);
});
// v0.295.0: Kurt's diagnostics — the envelope's ownership kept a removed
// character, because saving merged the map key by key.
test('v0.295.0 the envelope replaces each field whole, and the referee page republishes it on opening', async () => {
  const publish = await read('publish.js');
  const session = await import('node:fs/promises').then((fs) => fs.readFile(new URL('../src/play-session.js', import.meta.url), 'utf8'));
  assert.match(publish, /transaction\.set\(envelopeRef, payload, \{ mergeFields: Object\.keys\(payload\) \}\)/);
  assert.doesNotMatch(publish, /transaction\.set\(envelopeRef, [^;]*\{ merge: true \}\)/);
  assert.match(session, /setTimeout\(\(\) => \{ saveToCloud\(\); \}, 0\);/);
});

test('v0.315.4 a kind button\u2019s icon keeps its size inside the scene, where the map\u2019s own svg rule would stretch it', async () => {
  const css = await read('play.css');
  assert.match(css, /\.scene svg\.kind-icon \{[^}]*width: 14px; height: 14px;[^}]*padding: 0;/);
  assert.match(css, /\.map-head \.map-course \.button \{ white-space: nowrap; \}/);
});

test('v0.316.2 a redraw never lands while a button is held down, so a click is never lost to it', async () => {
  const play = await read('play.js');
  assert.match(play, /document\.addEventListener\('pointerdown', \(event\) => \{\n  if \(event\.button !== 0 \|\| !event\.target\?\.closest\?\.\(CLICKABLE\)\) return;\n  press\.held = true;/);
  assert.match(play, /function render\(\) \{\n  if \(press\.held \|\| document\.body\?\.dataset\.mapDragging\) \{ press\.owed = true; return; \}/);
  assert.match(play, /document\.addEventListener\('pointerup', releasePress, true\);/);
  assert.match(play, /setTimeout\(\(\) => render\(\), 0\);/);
});

test('v0.317.0 the ship chip opens the ship\u2019s sheet, which carries what the Ship panel did', async () => {
  const views = await read('play-views.js');
  const sheets = await read('sheets.js');
  assert.match(views, /onclick: \(\) => \(openSheet \? openSheet\('ship', s\.id\) : openDrawer\('ship'\)\)/);
  assert.match(sheets, /function shipStatus\(ship\)/);
  assert.match(sheets, /shipGauge\('Fuel', ship\.fuel, ' t'\)/);
});

test('v0.317.2 the lead travel button is light blue, and the departure checklist folds away with its verdict', async () => {
  const css = await read('play.css');
  const views = await read('play-views.js');
  assert.match(css, /--travel-fill: #bcd2e0; --travel-fill-ink: #1f3a4d;/);
  assert.match(views, /h\('details', \{ class: `checklist\$\{blocking\.length \? ' is-blocked' : risks\.length \? ' is-risk' : ''\}`/);
  assert.match(css, /\.checklist\.is-blocked > summary \{ background: var\(--danger-wash\)/);
});

test('v0.318.0 a week in jump space plays out a day at a time before it runs', async () => {
  const play = await read('play.js');
  assert.match(play, /const JUMP_DAY_MS = 500;/);
  assert.match(play, /if \(command === 'trip:jump-week' && playJumpWeek\(command\)\) return;/);
  assert.match(play, /const state = withJumpAnimation\(viewState\(\)\);/);
});

test('v0.323.0 the play map draws worlds the way the published maps do', async () => {
  const svg = await read('subsector-svg.js');
  const views = await read('play-views.js');
  const css = await read('play.css');
  assert.match(svg, /function appendBookWorld\(group, system, center\)/);
  assert.match(svg, /class: `world-dot \$\{hidden \? 'is-unknown' : hydro > 0 \? 'is-water' : 'is-dry'\}`/);
  assert.match(svg, /function zoneArc\(system, center\)/);
  assert.match(views, /worldStyle: 'book', edges: scene\.map\?\.edges \?\? \[\],/);
  assert.match(css, /\.world-dot\.is-dry \{ fill: var\(--map-dry\); \}/);
  assert.match(css, /\.system-hex\.current \.subsector-hex-shape \{ stroke: var\(--ink\); stroke-width: 3\.5; \}/);
});

test('v0.324.0 the map pans with a right-button drag and zooms with the wheel, on a camera kept per map', async () => {
  const views = await read('play-views.js');
  const play = await read('play.js');
  assert.match(views, /svg\.addEventListener\('wheel', \(event\) => \{\n    event\.preventDefault\(\);/);
  assert.match(views, /if \(!touch && event\.button !== 2\) return;/);
  assert.match(views, /svg\.addEventListener\('contextmenu', \(event\) => event\.preventDefault\(\)\);/);
  assert.match(views, /const CAMERA_KEY = 'traveller\.mapCamera';/);
  assert.match(play, /globalThis\.addEventListener\?\.\('traveller:map-drag-end'/);
});

test('v0.325.1 the map redraws at most once a frame while it pans or zooms, and is still while it moves', async () => {
  const views = await read('play-views.js');
  const css = await read('play.css');
  assert.match(views, /globalThis\.requestAnimationFrame\(run\)/);
  assert.match(css, /\.scene > svg\.map\.is-panning \* \{ animation-play-state: paused !important; pointer-events: none; \}/);
});

test('v0.326.0 subsector borders share one edge; referee tools in Settings and on the map', async () => {
  const svg = await read('subsector-svg.js');
  const views = await read('play-views.js');
  assert.match(svg, /const bottomOdd = subsectorHexCenter\(border\.firstColumn - firstColumn \+ 1, border\.firstRow - firstRow \+ 10/);
  assert.match(views, /onCommand\(`referee:move:\$\{selected\.id\}`\)/);
  assert.match(views, /handlers\.onCommand\('referee:clear-trip'\)/);
});

test('v0.326.2 the map head stays one line under the captions', async () => {
  const css = await read('play.css');
  assert.match(css, /\.map-head > span:first-child \{ flex: 1 1 auto; min-width: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; \}/);
  assert.match(css, /\.map-head \.map-rechart \{ width: auto; max-width: 11rem; \}/);
});
