...
test('v0.15.2 establishes a Traveller-first campaign hierarchy and removes duplicated play text', async () => {
  ...
  assert.match(html, /id="app-title" class="title">TRAVELLER</);
  assert.match(html, /id="app-subtitle" class="subtitle">v0\.96\.3</);
...
test('v0.60.0 rechecks player ownership and retires stale scene listeners', async () => {
  ...
  assert.match(index, /v0\.96\.3/);
  assert.match(playerPage, /PLAYER v0\.96\.3/);
});
...
test('v0.67.0 gives players a front door: their own characters, chargen, and a seat by invite', async () => {
  ...
  assert.match(html, /ENTER v0\.96\.3/);
...
test('v0.96.3 Manual Combat can request the range-line board, but never alongside a chosen scene', async () => {
  const html = await read('index.html');
  const app = await read('app.js');
  assert.match(html, /id="combat-spatial-mode"/);
  assert.match(html, /<option value="range-line">BOOK 1 RANGE LINE \(UNMAPPED\)<\/option>/);
  // Picking a scene always forces a real, mapped board — range-line has
  // nothing to draw a grid on top of.
  assert.match(app, /el\.combatSpatialMode\.disabled = Boolean\(scene\);/);
  assert.match(app, /if \(scene\) el\.combatSpatialMode\.value = 'scene';/);
  assert.match(app, /const spatialMode = scene \? 'scene' : \(el\.combatSpatialMode\?\.value === 'range-line' \? 'range-line' : 'scene'\);/);
