...
test('v0.15.2 establishes a Traveller-first campaign hierarchy and removes duplicated play text', async () => {
  ...
  assert.match(html, /id="app-title" class="title">TRAVELLER</);
  assert.match(html, /id="app-subtitle" class="subtitle">v0\.183\.0</);
...
test('v0.60.0 rechecks player ownership and retires stale scene listeners', async () => {
  ...
  assert.match(index, /v0\.183\.0/);
  assert.match(playerPage, /PLAYER v0\.183\.0/);
});
...
test('v0.67.0 gives players a front door: their own characters, chargen, and a seat by invite', async () => {
  ...
  assert.match(html, /ENTER v0\.183\.0/);
...
test('v0.96.0 a range-line encounter gets its own board, and never touches the camera', async () => {
  ...
  // v0.183.0: still no camera, and still inside the declared viewBox — but
  // the rows are the occupied bands rather than all sixteen.
  assert.match(fn, /const rowHeight = height \/ rowSlots;/);
...
test('the range line draws the occupied bands, named, and blows show on a melee token', async () => {
  const app = await readFile(new URL('../client/app.js', import.meta.url), 'utf8');
  const css = await readFile(new URL('../client/styles.css', import.meta.url), 'utf8');
  // Item 7: the occupied span with one band of headroom, and what was skipped
  // is stated rather than silently dropped.
  assert.match(app, /const lowest = Math\.max\(0, \(occupied\[0\] \?\? 0\) - 1\);/);
  assert.match(app, /EMPTY BAND\$\{slot\.bands === 1 \? '' : 'S'\}/);
  // Every band names itself and its Book 1 p.29 zone.
  assert.match(app, /label\.textContent = row === ENCOUNTER_RANGE_LINE_ESCAPE_BANDS \? 'ESCAPED' : `\$\{row\} \\u00b7 \$\{zoneForBand\(row\)\}`;/);
  assert.match(css, /\.range-line-row\.zone-medium \{/);
