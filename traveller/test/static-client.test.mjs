...
test('v0.15.2 establishes a Traveller-first campaign hierarchy and removes duplicated play text', async () => {
  ...
  assert.match(html, /id="app-title" class="title">TRAVELLER</);
  assert.match(html, /id="app-subtitle" class="subtitle">v0\.78\.0</);
...
test('v0.60.0 rechecks player ownership and retires stale scene listeners', async () => {
  ...
  assert.match(index, /v0\.78\.0/);
  assert.match(playerPage, /PLAYER v0\.78\.0/);
});
...
test('v0.67.0 gives players a front door: their own characters, chargen, and a seat by invite', async () => {
  ...
  assert.match(html, /ENTER v0\.78\.0/);
...
test('v0.77.0 the character sheet opens as a floating document window, on tested geometry', async () => {
  ...
  assert.match(app, /function createWindowController\(\{ element, titlebar, minimizeButton, closeButton, storageKey, onOpen \}\)/);
...
test('v0.78.0 separates scene tools, game tools, and the persistent activity rail', async () => {
  const app = await read('app.js');
  const html = await read('index.html');
  const css = await read('styles.css');
  const win = await read('../src/document-window.js');
  ...
  assert.match(css, /grid-template-columns: 88px minmax\(0, 1fr\) 392px;/);
  assert.match(css, /\.shell-rail \.scene-tabs,[\s\S]*?grid-template-columns: repeat\(2, 34px\);/);
  assert.match(css, /\.shell-sidebar \{[\s\S]*?grid-template-columns: 52px minmax\(0, 340px\);/);
  assert.match(css, /\.sidebar-tab-icon \{ font-size: 22px;/);
  assert.match(css, /\.sidebar-has-context \.sidebar-panel\[data-sidebar-panel="chat"\] \{ flex: 0 0 42%; \}/);
  assert.match(app, /const isChat = panel\.dataset\.sidebarPanel === 'chat';/);
