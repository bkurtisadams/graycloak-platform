...
test('client entry point uses ES modules and the terminal stylesheet', async () => {
  ...
  assert.match(html, /<script type="module" src="\.\/boot\.mjs"><\/script>/);
...
test('v0.15.2 establishes a Traveller-first campaign hierarchy and removes duplicated play text', async () => {
  ...
  assert.match(html, /id="app-title" class="title">TRAVELLER</);
  assert.match(html, /id="app-subtitle" class="subtitle">v0\.117\.1</);
...
test('v0.31.0 gives the centre three scene tabs, a status strip, and viewport-height columns', async () => {
  ...
  assert.match(strip, /id="toggle-system-details"/);
...
test('v0.60.0 rechecks player ownership and retires stale scene listeners', async () => {
  ...
  assert.match(index, /v0\.117\.1/);
  assert.match(playerPage, /PLAYER v0\.117\.1/);
});
...
test('v0.67.0 gives players a front door: their own characters, chargen, and a seat by invite', async () => {
  ...
  assert.match(html, /ENTER v0\.117\.1/);
...
test('v0.99.0 a port call shows WORLD, TRADE and JOBS at once; a takeover still claims the panel', async () => {
  const app = await read('app.js');
  const css = await read('styles.css');
  const html = await read('index.html');
  ...
  // The tab row becomes a jump list rather than a picker while stacked.
  assert.match(app, /panel\.scrollIntoView\(\{ block: 'start' \}\);/);
  assert.match(css, /\.port-stacked \.context-panel \{ display: block; \}/);
  assert.match(css, /\.port-stacked \.context-panel\[hidden\] \{ display: none; \}/);
});
...
test('v0.101.0 a desk card opens the drawer on its panel instead of selecting a hidden tab', async () => {
  ...
  assert.match(app, /const deskPanels = \{ port: el\.portServicesSection, trade: el\.commerceSection, jobs: el\.contractSection, situation: el\.situationSection \};/);
...
test('v0.103.0 crew can be assigned and released, which nothing could do before', async () => {
  ...
  // Candidates are party characters and rolled NPCs, minus anyone already
  // holding a role — one person, one role.
  assert.match(app, /return \[\.\.\.party, \.\.\.npcs\]\.filter\(\(entry\) => !held\.has\(entry\.id\)\);/);
  assert.match(app, /if \(intent === 'crew'\) \{ setSidebarTab\('vehicles'\); openCrewDialog\(\); return; \}/);
});
...
test('v0.117.0 the tool rail no longer costs a column it does not occupy', async () => {
  ...
  // Centred on the stage, not pinned to a top offset: the ship strip and the
  // map header sit above it and both change height.
  assert.match(css, /\.terminal\.shell \.shell-rail \{\n  left: 308px;\n  top: 50%; bottom: auto;\n  transform: translateY\(-50%\);\n\}/);
