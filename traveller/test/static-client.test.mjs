...
test('v0.15.2 establishes a Traveller-first campaign hierarchy and removes duplicated play text', async () => {
  ...
  assert.match(html, /id="app-title" class="title">TRAVELLER</);
  assert.match(html, /id="app-subtitle" class="subtitle">v0\.77\.1</);
...
test('v0.31.0 gives the centre three scene tabs, a status strip, and viewport-height columns', async () => {
  ...
  assert.match(app, /if \(encounterWorkspaceActive && activeSceneTab !== 'combat'\)/);
  assert.match(app, /el\.personnelSection\.hidden = !characterWindowOpen;/);
...
test('v0.77.0 opens the campaign character sheet as a floating session-persistent document window', async () => {
  const html = await read('index.html');
  const app = await read('app.js');
  const css = await read('styles.css');
  ...
  assert.match(app, /const CHARACTER_WINDOW_STORAGE_KEY = 'traveller\.character-window\.v1';/);
  assert.match(app, /function openCharacterWindow\(\)/);
  assert.match(app, /function closeCharacterWindow\(\)/);
  assert.match(app, /function toggleCharacterWindowMinimized\(\)/);
  assert.match(app, /window\.sessionStorage\.setItem\(CHARACTER_WINDOW_STORAGE_KEY/);
  assert.match(app, /titlebar\.setPointerCapture\(event\.pointerId\)/);
  assert.match(app, /if \(tab === 'character'\) \{ openCharacterWindow\(\); return; \}/);
  assert.match(css, /#personnel-section\.character-document-window \{/);
  assert.match(css, /resize: both;/);
  assert.match(css, /\.character-document-window\.is-minimized/);
});
...
test('v0.41.0 replaces the rosters with a combat tracker and declares from the token', async () => {
  ...
  assert.match(app, /const addCascade = \(label, build, disabled = false\) => \{/);
...
test('v0.60.0 rechecks player ownership and retires stale scene listeners', async () => {
  ...
  assert.match(index, /v0\.77\.1/);
  assert.match(playerPage, /PLAYER v0\.77\.1/);
});
...
test('v0.63.7 requires explicit referee selection and targeting', async () => {
  ...
  assert.match(app, /encounterExtraTargetIds\.has\(entry\.id\)/);
});
...
test('v0.67.0 gives players a front door: their own characters, chargen, and a seat by invite', async () => {
  ...
  assert.match(html, /ENTER v0\.77\.1/);
...
test('v0.71.0 sizes the board to the fight, draws tokens one square wide on both canvases, and tells a player why a move was refused', async () => {
  ...
  assert.match(doc, /const minimumMapMeters = document\.sceneId \? ENCOUNTER_SCENE_MIN_METERS : ENCOUNTER_MAP_MIN_METERS/);
...
test('v0.73.0 both pages draw the board with scene-canvas.js', async () => {
  ...
  // Neither page keeps its own board grid, camera, token base or token drag.
  ...
  assert.match(app, /titlebar\.addEventListener\('pointermove', move\)/, 'app.js uses pointer movement only for its document window');
...
test('v0.74.0 the active scene is staged, seen by players, and fought from the combat tracker', async () => {
  ...
  assert.match(app, /const start = makePortButton\('START COMBAT', \(\) => startCombatFromScene\(scene\)\);/);
...
test('v0.76.0 actors are folders you drag from onto a scene or combat setup', async () => {
  ...
  assert.match(app, /renderDirectoryFolders\(el\.directoryActors, directory\.actors, \{ emptyText: 'NO CHARACTERS OR NPCS', draggable: true \}\);/);
...
test('v0.76.2 the token menu survives a bad foe, explains why the last combatant on a side cannot be removed, and reinforcements can be dragged into a live fight', async () => {
  ...
  assert.match(app, /subItem\(submenu, `\$\{foe\.name\.toUpperCase\(\)\} \/ UNAVAILABLE`, \(\) => \{\}, true, error\?\.message \?\? String\(error\)\);/);
...
test('v0.76.3 the combat and subsector canvases are sized by flex-grow, not by an unresolved percentage height, and the masthead scrolls instead of hiding items', async () => {
  ...
  assert.match(app, /add\('REMOVE FROM ENCOUNTER', \(\) => removeCombatantFromActiveEncounter\(encounter\.identity\.id, combatant\.id\), resolved \|\| lastOnSide\);/);
});
