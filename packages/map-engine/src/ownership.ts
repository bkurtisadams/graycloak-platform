...
import type { Axial, ParentCoord, Bbox } from './types.js';

const _ownerCache = new Map<string, ParentCoord>();
 ...
 */
export function ownerOf(Q: number, R: number): ParentCoord | null {
  const key = `${Q}_${R}`;
 ...
 */
export function ownedByParent(col: number, row: number): Axial[] {
 ...
 */
export function cellsInAxialBbox(bbox: Bbox | null | undefined): Axial[] {
  const out: Axial[] = [];
  if (!bbox) return out;
