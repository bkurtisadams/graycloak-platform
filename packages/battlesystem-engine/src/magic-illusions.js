...
function illusionProfile(entry = {}, opts = {}) {
  ...
  if (key === 'hallucinatory terrain') {
    const illusionist = metaClass === 'illusionist';
    const sourceLength = illusionist ? 4 + casterLevel : casterLevel;
    const rangeIn = illusionist ? 2 + 2 * casterLevel : 2 * casterLevel;
    return {
      ...base, hallucinatoryTerrain: true, sourceClass: metaClass, casterLevel,
      rangeIn, shape: 'square', sourceLength, saveType: 'none', concentration: false,
      movableArea: false, terrainContactEnds: true, dispellable: true,
      castingTimeText: illusionist ? '5 rounds' : '1 turn', durationText: 'Until dispelled or contacted by an intelligent creature',
      sensory: ['visual'],
      label: `Hallucinatory Terrain · ${illusionist ? 'Illusionist 3' : 'Magic-User 4'} · range ${rangeIn}″ · ${sourceLength}″ × ${sourceLength}″ source square · no save · ends on intelligent contact or Dispel Magic`
    };
  }
  ...
  const basic = key === 'phantasmal force', improved = key === 'improved phantasmal force', spectral = key === 'spectral force', permanent = key === 'permanent illusion';
  const illusionist = metaClass === 'illusionist';
  const sourceAreaSqIn = (basic && !illusionist ? 8 : 4) + casterLevel;
  const rangeIn = permanent ? casterLevel : (basic && !illusionist ? 8 : 6) + casterLevel;
  const castingTimeSegments = permanent ? 6 : spectral ? 3 : improved ? 2 : illusionist ? 1 : 3;
  const sensory = spectral || permanent ? ['visual','sound','smell','thermal'] : improved ? ['visual','minor-sound'] : ['visual'];
  const postConcentrationRounds = spectral ? 3 : improved ? 2 : 0;
  return {
    ...base,
    ...
    minimalConcentration: improved || spectral,
    concentrationMoveFactor: improved || spectral ? 0.5 : 0,
    postConcentrationRounds,
    permanent,
    dispellable: permanent,
    movableArea: !permanent,
    visualOnly: basic,
    sensory,
    castingTimeSegments,
