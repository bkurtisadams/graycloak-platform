// @graycloak/battlesystem-engine spells.js v0.7.1 - 2026-08-26
...
const DAOUD_LANTHORN = Object.freeze({
  ...
  // 30 feet = 3 game inches, while the special 10-foot zone = 1 game inch. Outdoors
  // those same 3" / 1" tabletop distances represent 30 / 10 yards respectively.
  rangeFeet:30,
  rangeGameIn:3,
  rangeIn:3,
  closeFeet:10,
...
function sourcePreset(entry={},opts={}) {
  ...
  if(key==='holdMonster')return {...daoudSinglePrismBase(key,'Ruby',`Hold Monster · one victim · 30′ source range = 3″ AD&D/BATTLESYSTEM · effective level ${L} · 1 round/level (${L} rounds) · one-target save -3 beyond 10′; within 10′ / 1″ no save and magic resistance does not protect · 5 years fuel`),saveType:'sp',saveEffect:'negates',saveMod:-3,lockSaveType:true,lockSaveMod:true,statusEffect:'hold',durationRounds:L};
  if(key==='holdPerson')return {...daoudSinglePrismBase(key,'Ruby',`Hold Person · one victim · 30′ source range = 3″ AD&D/BATTLESYSTEM · effective level ${L} · 2 rounds/level (${2*L} rounds) · one-target save -3 beyond 10′; within 10′ / 1″ no save and magic resistance does not protect · 5 years fuel`),saveType:'sp',saveEffect:'negates',saveMod:-3,lockSaveType:true,lockSaveMod:true,statusEffect:'hold',durationRounds:2*L};
  if(key==='haste')return {...daoudSinglePrismBase(key,'Oriental Emerald',`Haste · one victim · 30′ source range = 3″ AD&D/BATTLESYSTEM · effective level ${L} · ${3+L} rounds · no save · 5 years fuel`),saveType:'none',saveEffect:'none',lockSaveType:true,statusEffect:'haste',durationRounds:3+L};
  if(key==='colorSpray')return {...daoudSinglePrismBase(key,'Diamond',`Color Spray · one victim · 30′ source range = 3″ AD&D/BATTLESYSTEM · effective level ${L} · PHB level/HD outcome and special save remain spell-specific · within 10′ / 1″ the artifact allows no save/MR · 5 years fuel`),saveType:'none',saveEffect:'none',lockSaveType:true,specialResolver:'colorSpray'};
  if(key==='fear')return {...daoudSinglePrismBase(key,'Sapphire',`Fear · one victim · 30′ source range = 3″ AD&D/BATTLESYSTEM · effective level ${L} · save negates beyond 10′; within 10′ / 1″ no save/MR · panic begins next round and lasts ${L} rounds · 5 years fuel`),saveType:'sp',saveEffect:'negates',saveMod:0,lockSaveType:true,lockSaveMod:true,statusEffect:'fear',onsetRounds:1,durationRounds:L};
  if(key==='emotionRage')return {...daoudSinglePrismBase(key,'Oriental Amethyst',`Emotion (Rage) · one victim · 30′ source range = 3″ AD&D/BATTLESYSTEM · effective level ${L} · save negates beyond 10′; within 10′ / 1″ no save/MR · rage modifiers/concentration remain spell-specific · 5 years fuel`),saveType:'sp',saveEffect:'negates',saveMod:0,lockSaveType:true,lockSaveMod:true,specialResolver:'emotionRage'};
  if(key==='flameStrike')return {...daoudSinglePrismBase(key,'Jacinth',`Flame Strike · one victim · 30′ source range = 3″ AD&D/BATTLESYSTEM · effective level ${L} · 6d8 fire · save for half beyond 10′; within 10′ / 1″ no save/MR · 5 years fuel`),saveType:'sp',saveEffect:'half',saveMod:0,lockSaveType:true,lockSaveMod:true,damageExpr:'6d8',damageTag:'fire'};
  return null;
}
