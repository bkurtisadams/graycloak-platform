export const CLASS_RULE_SOURCE = Object.freeze({
...
export const CLASS_MINIMUMS = Object.freeze({
    fighter: { str: 9 },
...
export const SINGLE_CLASS_OPTIONS = Object.freeze({
...
export const MULTICLASS_COMBINATIONS = Object.freeze({
    human: [],
    dwarf: [
        ['fighter', 'thief'],
    ],
...
const dwarfFighterCap = (scores) => scores.str >= 18 ? 9 : scores.str === 17 ? 8 : 7;
const elfFighterCap = (scores) => scores.str >= 18 ? 7 : scores.str === 17 ? 6 : 5;
const elfMagicUserCap = (scores) => scores.int >= 18 ? 11 : scores.int === 17 ? 10 : 9;
const gnomeFighterCap = (scores) => (scores.str >= 18 ? 6 : 5);
const gnomeIllusionistCap = (scores) => scores.int >= 17 && scores.dex >= 17 ? 7 : scores.int >= 17 || scores.dex >= 17 ? 6 : 5;
const halfElfFighterCap = (scores) => scores.str >= 18 ? 8 : scores.str === 17 ? 7 : 6;
const halfElfMagicUserCap = (scores) => scores.int >= 18 ? 8 : scores.int === 17 ? 7 : 6;
const halflingFighterCap = (scores) => scores.str >= 18 ? 6 : scores.str === 17 ? 5 : 4;
const halfOrcThiefCap = (scores) => scores.dex >= 18 ? 8 : scores.dex === 17 ? 7 : 6;
export const RACE_LEVEL_CAPS = Object.freeze({
    dwarf: {
        cleric: -8,
        druid: 0,
...
export function getMissingClassMinimums(classId, scores) {
    const result = {};
    const minimums = CLASS_MINIMUMS[classId];
    for (const [ability, required] of Object.entries(minimums)) {
        const actual = scores[ability];
        if (actual < required)
            result[ability] = { required, actual };
    }
    return result;
}
export function meetsClassMinimums(classId, scores) {
    return Object.keys(getMissingClassMinimums(classId, scores)).length === 0;
}
export function getRaceLevelCap(raceId, classId, scores) {
    const rule = RACE_LEVEL_CAPS[raceId][classId];
    return typeof rule === 'function' ? rule(scores) : rule;
}
export function getSingleClassEligibility(raceId, classId, scores) {
...
export function canPlaySingleClass(raceId, classId, scores) {
    return getSingleClassEligibility(raceId, classId, scores).allowed;
}
export function getValidMulticlassCombinations(raceId, scores) {
    return MULTICLASS_COMBINATIONS[raceId].filter((combination) => combination.every((classId) => {
        if (!meetsClassMinimums(classId, scores))
            return false;
...
export function isValidMulticlassCombination(raceId, classIds, scores) {
    const wanted = [...classIds].sort().join('|');
    return getValidMulticlassCombinations(raceId, scores).some((combination) => [...combination].sort().join('|') === wanted);
}
