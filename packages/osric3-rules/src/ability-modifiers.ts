...
import type { AbilityScores, RuleSource } from './types.js';

export const ACTIVE_ABILITY_MODIFIER_RULE_SOURCE: RuleSource = Object.freeze({
  ruleset: 'legacy-adnd-1e',
  section: 'Ability modifiers currently used by the GCC character generator',
  auditStatus: 'legacy-import',
  note:
    'Centralizes the active Constitution hit-point adjustment, Charisma NPC reaction adjustment, and Strength weight allowance. The remaining displayed ability tables still require extraction and OSRIC 3.0 verification.',
});

export interface ActiveAbilityModifiers {
  readonly constitutionHitPointAdjustment: number;
  readonly charismaNpcReactionAdjustment: number;
  readonly strengthWeightAllowance: number;
}

export function getConstitutionHitPointAdjustment(constitution: number): number {
...
export function getCharismaNpcReactionAdjustment(charisma: number): number {
...
export function getActiveAbilityModifiers(
