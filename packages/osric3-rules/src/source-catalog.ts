import type { RuleAuditStatus, RuleSource } from './types.js';
...
export interface RuleSourceRecord extends RuleSource {
  readonly id: RuleSourceId;
  readonly module: string;
}

function source(record: RuleSourceRecord): RuleSourceRecord {
...
export const RULE_SOURCE_CATALOG: Readonly<Record<RuleSourceId, RuleSourceRecord>> = Object.freeze({
  ...
  'attack-matrix': source({ id: 'attack-matrix', module: 'combat', ruleset: 'osric-3.0', section: 'Class and monster attack matrices, repeating-20 rule, weapon-vs-AC adjustments', book: 'OSRIC 3.0 Player Guide (2026-01-22 rev), Tables 1.3.1.4d-1.3.10.4f; Gamemaster Guide Table 2.1.2a', page: 43, auditStatus: 'verified-osric3', note: 'All ten class tables verified cell-by-cell against print (227 cells) and the monster matrix against Table 2.1.2a (16 rows). OSRIC 3.0 adopted as the ruling document: fighter family improves one point per level (floor 1 at 20+), cleric 19+ is 9, magic-user/illusionist 16-20 is 14, monster 2-3+ is 16 and 6-7+ is 13. Not printed in the Player Guide and retained from 1e: magic-user 21+ (11) and thief 21+ (10).' }),
  ...
  'saving-throws': source({ id: 'saving-throws', module: 'saving-throws', ruleset: 'osric-3.0', section: 'Class saving throws', book: 'OSRIC 3.0 Player Guide (2026-01-22 rev), Tables 1.3.1.4c-1.3.10.4b', page: 41, auditStatus: 'verified-osric3', note: 'All ten class tables verified cell-by-cell against print (325 cells). Corrections: paladins and monks now read their own printed tables rather than the fighter table less two and the thief table; the fighter family gains its 19+ band and a corrected level-0 row; thief rod/staff/wand and spell columns corrected from level 5 up. Not printed and therefore dropped: magic-user 21+ and thief 21+ rows, which now hold the highest printed band.' }),
...
export function getRuleSourceRecord(id: RuleSourceId): RuleSourceRecord {
  return RULE_SOURCE_CATALOG[id];
}

export function listRuleSourceRecords(auditStatus?: RuleAuditStatus): readonly RuleSourceRecord[] {
