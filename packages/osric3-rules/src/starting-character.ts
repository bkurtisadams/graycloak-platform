...
import { rollDice, rollDie, rollPercentile, type RandomSource } from './dice.js';
import {
  ABILITY_IDS,
  type AbilityId,
  type AbilityScores,
  type AgeCategoryIndex,
  type ClassId,
  type GenderId,
  type RaceId,
  type RuleSource,
...
export const STARTING_CHARACTER_RULE_SOURCE: RuleSource = Object.freeze({
...
interface DiceExpression {
  base: number;
  dice: readonly [count: number, sides: number];
}

export const STARTING_AGE: Readonly<
  Record<RaceId, Readonly<Partial<Record<ClassId, DiceExpression>>>>
> = Object.freeze({
  human: {
...
export type AgeBrackets = readonly [number, number, number, number, number, number];

export const AGE_BRACKETS: Readonly<Record<AgeRaceId, AgeBrackets>> = Object.freeze({
...
export const AGE_ADJUSTMENTS: readonly Readonly<AbilityScores>[] = Object.freeze([
...
function getAgeBrackets(raceId: AgeRaceId): AgeBrackets {
  return AGE_BRACKETS[raceId] ?? AGE_BRACKETS.human;
}

function getAgeInterval(raceId: AgeRaceId): number {
  const brackets = getAgeBrackets(raceId);
  const venerableSpan = brackets[5] - brackets[4];
  if (venerableSpan < 100) return 1;
  if (venerableSpan <= 250) return 10;
  return 20;
}

export function rollStartingAge(
  raceId: RaceId,
  classId: ClassId,
  random?: RandomSource,
): number {
  const entry = STARTING_AGE[raceId][classId];
  if (!entry) return getAgeBrackets(raceId)[0] + rollDice(1, 4, random);
  return entry.base + rollDice(entry.dice[0], entry.dice[1], random);
}

export function getAgeCategory(raceId: AgeRaceId, age: number): AgeCategoryIndex {
  const brackets = getAgeBrackets(raceId);
  if (age < brackets[1]) return 0;
  if (age < brackets[2]) return 1;
  if (age < brackets[3]) return 2;
  if (age < brackets[4]) return 3;
  return 4;
}

export function getCumulativeAgeAdjustments(category: AgeCategoryIndex): AbilityScores {
  const totals: AbilityScores = { str: 0, int: 0, wis: 0, dex: 0, con: 0, cha: 0 };
  for (let index = 0; index <= category; index += 1) {
    const adjustment = AGE_ADJUSTMENTS[index]!;
    for (const abilityId of ABILITY_IDS) totals[abilityId] += adjustment[abilityId];
  }
  return totals;
}
...
export function applyAgeAdjustments(
  ...
  for (const abilityId of ABILITY_IDS) {
    ...
    const maximum = abilityId === 'wis'
      ...
      : racialMaximum;
    result[abilityId] = Math.max(
      minimum,
      Math.min(maximum, result[abilityId] + adjustments[abilityId]),
    );
  }
...
export function rollMaximumAge(raceId: AgeRaceId, random?: RandomSource): number {
  const brackets = getAgeBrackets(raceId);
...
export interface AgingSubject extends AbilityScores {
  age: number;
  race: RaceId;
}

export interface UnnaturalAgingResult {
  newAge: number;
  oldCategory: AgeCategoryIndex;
  newCategory: AgeCategoryIndex;
  statChanges: Partial<Record<AbilityId, number>>;
}

export function applyUnnaturalAging(subject: AgingSubject, years: number): UnnaturalAgingResult {
  ...
  const statChanges: Partial<Record<AbilityId, number>> = {};
...
interface HeightWeightEntry {
  hAvg: number;
  hUnder: readonly [number, number];
  hOver: readonly [number, number];
  wAvg: number;
  wUnder: readonly [number, number];
  wOver: readonly [number, number];
}

export const HEIGHT_WEIGHT: Readonly<
  Record<GenderId, Readonly<Record<RaceId, HeightWeightEntry>>>
> = Object.freeze({
  male: {
...
interface HeightWeightDistribution {
  hUnder: number;
  hOver: number;
  wUnder: number;
  wOver: number;
...
export const HEIGHT_WEIGHT_DISTRIBUTION: Readonly<Record<RaceId, HeightWeightDistribution>> =
  Object.freeze({
...
export interface HeightWeightResult {
  height: number;
  weight: number;
}
...
export function rollHeightWeight(
  ...
  if (heightRoll <= distribution.hUnder) {
    ...
    if (varianceRoll <= 30) height = table.hAvg - variance;
    else if (varianceRoll >= 71) height = table.hAvg + variance;
    else height = table.hAvg;
  }
  ...
  if (weightRoll <= distribution.wUnder) {
    ...
    if (varianceRoll <= 30) weight = table.wAvg - variance;
    else if (varianceRoll >= 71) weight = table.wAvg + variance;
    else weight = table.wAvg;
  }
...
export const SECONDARY_SKILLS: readonly SecondarySkillEntry[] = Object.freeze([
...
function findSecondarySkill(roll: number): string | null {
  return SECONDARY_SKILLS.find((entry) => roll >= entry.range[0] && roll <= entry.range[1])?.name ?? null;
}

function rollSecondarySkillEntry(random?: RandomSource): string | null {
  return findSecondarySkill(rollDie(85, random));
}

export function rollSecondarySkills(random?: RandomSource): readonly string[] {
  const percentile = rollPercentile(random);
  if (percentile >= 86) {
...
export function rollStartingGold(classId: ClassId, random?: RandomSource): number {
  switch (classId) {
    case 'fighter':
    case 'paladin':
...
export const CLASS_HIT_DICE: Readonly<Record<ClassId, number>> = Object.freeze({
  fighter: 10,
  cleric: 8,
  thief: 6,
  'magic-user': 4,
...
export const FIGHTER_CLASS_IDS: readonly ClassId[] = Object.freeze(['fighter', 'paladin', 'ranger']);

export function rollPercentileStrength(
  classId: ClassId,
  strength: number,
  random?: RandomSource,
): number {
...
export function constitutionHitPointBonus(constitution: number): number {
...
export function rollStartingHitPoints(
  classId: ClassId,
  constitution: number,
  random?: RandomSource,
): number {
  let bonus = constitutionHitPointBonus(constitution);
  if (!FIGHTER_CLASS_IDS.includes(classId) && bonus > 2) bonus = 2;
...
export function getClassExperienceBonus(
  classId: ClassId,
  scores: AbilityScores,
  ...
  let qualifies = false;
  switch (classId) {
    case 'fighter': qualifies = scores.str > 15; break;
    case 'paladin': qualifies = scores.str > 15 && scores.wis > 15; break;
    case 'ranger': qualifies = scores.str > 15 && scores.int > 15 && scores.wis > 15; break;
    case 'cleric': qualifies = scores.wis > 15; break;
    case 'druid': qualifies = scores.wis > 15 && scores.cha > 15; break;
    case 'magic-user': qualifies = scores.int > 15; break;
    case 'thief': qualifies = scores.dex > 15; break;
    case 'illusionist':
    case 'assassin':
...
export interface StartingCharacterBuildRequest {
  raceId: RaceId;
  classId: ClassId;
  baseScores: AbilityScores;
  genderId?: GenderId;
  preRolledSecondarySkills?: readonly string[];
  random?: RandomSource;
}

export type StartingCharacterBuildResult =
  | { valid: false; error: string }
  | {
      valid: true;
      racialScores: AbilityScores;
      finalScores: AbilityScores;
      age: number;
      ageCategory: AgeCategoryIndex;
      ageCategoryName: (typeof AGE_CATEGORY_NAMES)[number];
      genderId: GenderId;
      height: number;
      weight: number;
...
export function buildStartingCharacter(
  request: StartingCharacterBuildRequest,
): StartingCharacterBuildResult {
  ...
  if (!eligibility.allowed) {
    if (eligibility.reason === 'below-minimums') {
      const first = Object.entries(eligibility.missingMinimums)[0] as
        | [AbilityId, { required: number; actual: number }]
        | undefined;
      if (first) {
